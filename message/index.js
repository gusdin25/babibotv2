...
const tanggal = moment.tz('Asia/Makassar').format('DD-MM-YYYY')
/********** END OF UTILS **********/
...
module.exports = msgHandler = async (bocchi = new Client(), message) => {
    try {
        ...
        switch (command) {
            ...
            case 'tos':
                if (!isRegistered) return await bocchi.reply(from, ind.notRegistered(), id)
                await bocchi.sendLinkWithAutoPreview(from, 'https://github.com/gusdin25/babibot', ind.tos(ownerNumber))
            break
            case 'join':
                if (!isRegistered) return await bocchi.reply(from, ind.notRegistered(), id)
